...
export async function boot() {
  ...
  if (!remoteReady.ok) {
    enableLocalFallback(remoteReady.error, sessionState);
  }
}
...
export function runTournamentWrite(action) {
  ...
  tournamentWriteQueue = queued.catch(error => {
    console.error('[sync] Score non synchronise:', error);
    setStatus('Score non synchronise — verifie ta connexion.', 'error');
  });
  return queued;
}
